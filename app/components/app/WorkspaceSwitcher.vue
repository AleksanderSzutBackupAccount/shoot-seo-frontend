...
const items = computed<DropdownMenuItem[][]>(() => [
  ...
  [{ label: t('shell.workspaceNew'), icon: 'i-lucide-plus', to: '/workspaces/new' }],
])
</script>
...
<template>
  <UDropdownMenu :items="items" :ui="{ content: 'w-60' }">
    ...
    >
      ...
      <span class="truncate text-sm" style="color: var(--ink); font-weight: 500">{{ current?.name ?? $t('shell.workspaceSelect') }}</span>
